...
static void client_arp_whohas(byte *);
...
static byte servermacaddr[6];
static byte servermac_known = 0;
...
void EtherCard::makeUdpReply (char *data,word datalen,word port) {
  if (datalen>560)
    datalen = 560;
...
void EtherCard::udpPrepare (word sport, byte *dip, word dport) {
  setMACandIPs(servermacaddr, dip);
...
void EtherCard::sendArpRequest(byte *ip) {
...
static byte client_store_server_mac() {
  if (memcmp(gPB + ETH_ARP_SRC_IP_P, EtherCard::serverip, 4) != 0)
    return 0;
  EtherCard::copyMac(servermacaddr, gPB + ETH_ARP_SRC_MAC_P);
  servermac_known = 1;
  return 1;
}
...
byte EtherCard::serverMacKnown() {
  return servermac_known;
}
...
bool EtherCard::is_myIp(word plen) {
  return eth_type_is_arp_and_my_ip(plen)==0;
}
